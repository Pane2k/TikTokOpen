...
class TikTokDataService:
    def __init__(self, hashtag="костиккакто", userlist_link="Data/TXT/Cacto0o.txt", websocket_host="localhost", websocket_port=8001, refresh_interval=300):
    ...
    async def _get_tiktok_data(self):
       try:
            data = await getInfo(self.hashtag, self.userlist_link)
        ...
        for user_data in data["userStats"]:
            ...
            for new_user_data in data_dict["userStats"]:
                ...
                if user_data["username"] == new_user_data["username"]:
                    ...
                    self.logger.debug(f"Updated user: {user_data['username']}")
                    break
            else:
                data_dict.get("userStats").append(user_data)
                self.logger.info(f"newUser {user_data['username']}")
    ...
    async def _fetch_tiktok_data_periodically(self):
        ...
        while True:
            ...
            self.logger.debug("Starting fetch_tiktok_data_periodically")
            ...
            lastResult = {
                "videoCount": int(result["videoCount"]) - int(self.data["total_videos_with_tag"]),
                "viewCount": int(result["viewCount"]) - int(self.data["total_views"]),
            }
    ...
    def _update_data_periodically(self):
            self.logger.info("Starting update_data_periodically")
        ...
        while True:
            try:
                ...
                if self.data_to_resive is not None:
                    ...
                    timeToRestart = int((self.last_reload_time + self.refresh_interval) - time.time())
    ...
    async def _get_original_views(self, api: TikTokApi, hashtag):
        try:
          ...
          saveJson("Data/JSON/RawHashtagStats.json", {"videoCount": videoCount, "viewCount": viewCount})
        except Exception as e:
          self.logger.error(f"Error getting original views: {e}")
        ...
        while True:
            try:
              ...
              async with TTapi as apia:
                   ...
                   self.logger.info(f"lastResult: {lastResult}")
                   saveJson("Data/JSON/DataToResive.json", lastResult)
                   self.data_to_resive = lastResult
                ...
                self.logger.error(f"error in _get_views_async {e}")
                try:
                    await apia.close_sessions()
                    await apia.stop_playwright()
                except:
    ...
    async def run(self):
        async with websockets.serve(self._handler, self.websocket_host, self.websocket_port):
          self.logger.info(f"Server started on ws://{self.websocket_host}:{self.websocket_port}")

            # Start threads
          threadTikTokInfo = threading.Thread(target=asyncio.run, args=(self._fetch_tiktok_data_periodically(),))
